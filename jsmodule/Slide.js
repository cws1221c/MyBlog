...
export default class Slide {
    ...
    endSlide(){
        this.sliding = false;
        this.current == 0 ? this.slideBtn.eq(0).attr("disabled", true) : this.slideBtn.eq(0).attr("disabled", false);
        this.current == this.sList.length - 1 ? this.slideBtn.eq(1).attr("disabled", true) : this.slideBtn.eq(1).attr("disabled", false);
    }
}